...
| `POST` | `/save_memory` | store a distilled note — `content`, the required `author` and `tags`, `kind`, and the optional id of a prior note to archive; refused with 409 when a near-identical active note exists unless `supersedes` names it or `allow_similar` is set |
