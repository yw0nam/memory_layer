...
- **Low-signal data never reaches the embedding path.** Agent-authored notes arrive distilled through the MCP `save_memory` tool (validation, dedup, supersede) and are stored without embedding raw text. Documents go through deterministic chunking and a junk gate before embedding, with tags supplied by the uploader; the chat model is called in exactly two places — the CSV branch, to summarize a sampled table into one card, and the save_memory content gate, to refuse a note that restates a tracker artefact, reports progress, or describes a file; the chat provider is chosen from API keys in `.env` (z.ai, OpenAI, Anthropic), falling back to the configured vLLM endpoint when none is set. Raw transcripts and raw files are never embedded directly. A tabular document's data rows are stored as structured rows in `doc_rows`, read by SQL only — never embedded, never in search results (ADR-0001). Agent-authored messages (the `messages` table: general signals and handoff snapshots) are rendered to canonical Markdown and stored unembedded; they are read by address and claimed once, never ranked (ADR-0002).
- **Source-specific knowledge lives only in source adapters (`adapters/`).** `ingest/`, the storage schema, retrieval (`retrieval/`), and consumers (`serve/`) are source-agnostic. The `memory_chunks`, `code_chunks`, `doc_rows`, and `messages` tables are the only contract between the write side and the read side — adding a new source must not change retrieval or serving code.
...
- **Docs: current-state only.** Write what the system *is*, declaratively, matching the code — no change-narrative, no PR/issue numbers as prose, no future/unbuilt work (`PostToolUse` docs guard enforces this). `CHANGELOG.md` is the one sanctioned exception: it is generated from git history, one section per released version, and is never hand-edited. Versioning is SemVer on `0.x`: `minor` for features and breaking changes, `patch` for fixes.
