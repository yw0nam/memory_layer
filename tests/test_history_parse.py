...
def test_parser_contract():
    fixture = "\n".join([
        ...
        line(type="user", sessionId="s1", timestamp="2026-06-25T05:08:12.347Z",
             message={"content": "실제 질문"}),
        line(type="assistant", sessionId="s1", timestamp="2026-06-25T05:08:13.347Z",
             message={"content": [{"type": "thinking", "thinking": "숨은 사고"},
                                  {"type": "text", "text": "확인"},
                                  {"type": "tool_use", "name": "Read"}]}),
        line(type="user", sessionId="s1", message={"content": [
            {"type": "tool_result", "content": "도구 출력", "is_error": True}]}),
        line(type="assistant", sessionId="s1", isSidechain=True,
...
def msg(role, text, seconds, error=False):
    return Message(role, text, datetime(2026, 1, 1, tzinfo=timezone.utc).timestamp() + seconds,
                   "s1", tool_error=error)


def test_burst_group_and_length_filter():
    bursts = group_bursts([
        msg("user", "가" * 110, 0), msg("user", "나" * 100, 5),
        msg("assistant", "짧음", 10), msg("user", "다" * 199, 20),
    ])
    assert len(bursts) == 1
...
def test_burst_social_signals():
    ...
    assert group_bursts([msg("assistant", "x" * 200, 0),
                         msg("user", "y" * 200, 120)])[0].social_weight == 1.5
